...
func TestSyncStatusPageAllowsHashNavigation(t *testing.T) {
	...
	// finance 来自 main；model-statistics/customer-health 来自本地功能，两边只增不减。
	if !strings.Contains(pageHTML, `/^(sync|model|server|capacity|model-statistics|usage|stability|channels|finance|logchain|alerts|group-governance|customer-health)$/`) {
