...
const ST_HEADERS={
  ...
  stability:{title:'稳定性报表',subtitle:'客户端确认 / 服务端协议 / 历史日志推断 · 问题证据',icon:'shield'},
...
const st={inited:false,loaded:false,view:'history',layer:'delivery',days:1,custom:null,preset:'',filters:{vendor:'',group:'',channel:'',model:''},allFilters:null,report:null,evidence:null,abort:null,evidenceAbort:null,problemAbort:null,drawerAbort:null,edgeAbort:null,edgeReport:null,generation:0,detailPromises:new Map(),detailControllers:new Map(),detailLoading:new Set(),expanded:new Set(),chart:null,drawerChart:null,edgeChart:null,drawer:null,drawerTab:'run',lastFocus:null};
...
function init(){
  ...
  for(const id of ['stVendor','stGroup','stChannel','stModel'])$(id)?.addEventListener('change',()=>{readFilters();loadReport();loadDeliveryEvidence()});
  $('stFilterReset')?.addEventListener('click',()=>{st.filters={vendor:'',group:'',channel:'',model:''};renderFilterOptions();loadReport();loadDeliveryEvidence()});
  $('stRefresh')?.addEventListener('click',()=>{loadReport();loadDeliveryEvidence()});
  $('stProblemRefresh')?.addEventListener('click',loadProblems);
  $('stRequestLookupBtn')?.addEventListener('click',()=>{const value=$('stRequestLookup')?.value?.trim();if(value)openDeliveryTimelineByRequestID(value)});
  $('stRequestLookup')?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();$('stRequestLookupBtn')?.click()}});
  $('stProblemBody')?.addEventListener('click',e=>{const row=e.target.closest('[data-delivery-request]');if(row)openDeliveryTimeline(row.dataset.deliveryRequest)});
...
function reloadActiveLayer(){loadDeliveryEvidence();if(st.layer==='edge')loadEdge();else loadReport()}

async function loadDeliveryEvidence(){
  if(st.evidenceAbort)st.evidenceAbort.abort();st.evidenceAbort=new AbortController();
  try{
    const res=await fetch('/stability/delivery-evidence?'+queryParams({protocol:'openai_responses'}),{headers:{Accept:'application/json'},signal:st.evidenceAbort.signal});
    if(res.status===401){location.href='/login';return}const d=await res.json();if(!res.ok)throw new Error(d.error||`HTTP ${res.status}`);
    st.evidence=d;renderDeliveryEvidence(d);
  }catch(e){if(e.name!=='AbortError')errorBox($('stEvidenceOverview'),e.message)}
}
function evidencePct(v){return v==null?'不可判断':(+v).toFixed(2)+'%'}
function renderDeliveryEvidence(d){
  const el=$('stEvidenceOverview');if(!el)return;const s=d.summary||{},client=d.client_metric_available;
  const actualRange=d.from_ms&&d.to_ms?`${new Date(d.from_ms).toLocaleString('zh-CN',{hour12:false})} 至 ${new Date(d.to_ms).toLocaleString('zh-CN',{hour12:false})}`:'—';
  const headline=!client?'未接入受控客户端':s.technical_outcomes?evidencePct(s.success_rate):`仅主动取消 ${nfmt(s.cancelled)} 条`;
  const detail=!client?'无法判断用户侧技术结果；不会用 NewAPI 日志或服务端结果补值':`${s.evidence_sufficient?'当前受控样本可判断':'样本或覆盖不足，不外推全站'} · 技术结果 ${nfmt(s.technical_outcomes)} · 回传 ${nfmt(s.outcomes)}/${nfmt(s.started)} · 覆盖 ${evidencePct(s.coverage)} · 主动取消 ${nfmt(s.cancelled)} · 孤儿结果 ${nfmt(s.orphan_outcomes)}`;
  el.innerHTML=`<section class="stability-panel"><div class="stability-panel-head"><div><h3>可选客户端技术结果</h3><p>受控客户端可直接上报 Monitor；不要求、也不修改 NewAPI 或 Nginx</p></div><span class="muted">${actualRange}</span></div>`+
    `<div class="stability-evidence-grid"><article class="stability-evidence-card ${s.evidence_sufficient&&s.success_rate===100?'good':s.evidence_sufficient?'bad':''}"><small>客户端确认成功率</small><b>${headline}</b><em>${detail}</em></article>`+
    `<article class="stability-evidence-card"><small>客户端回传覆盖</small><b>${evidencePct(s.coverage)}</b><em>开始 ${nfmt(s.started)} · 终态 ${nfmt(s.outcomes)} · 失败 ${nfmt(s.failures)}</em></article>`+
    `<article class="stability-evidence-card"><small>显式逻辑请求 · 首次 / 最终成功</small><b>${nfmt(s.first_successes)} / ${nfmt(s.eventual_successes)}</b><em>逻辑链 ${nfmt(s.logical_requests)} · 发生重试 ${nfmt(s.retried_requests)}</em></article>`+
    `<article class="stability-evidence-card ${s.conflicts?'bad':''}"><small>客户端证据冲突</small><b>${nfmt(s.conflicts)}</b><em>同一请求出现互斥终态时 fail-closed，不计入成功。</em></article></div>`+
    `<div class="stability-evidence-note">边界：Monitor 不修改 NewAPI，因此不声称自动知道 response.completed、Flush 或客户端是否接收。下方“历史日志推断”是独立旧口径。</div></section>`;
}
...
function renderReport(){
  ...
  body.innerHTML=`<section class="stability-kpis" id="stKpis"></section>
    ...
    <section class="stability-panel"><div class="stability-panel-head"><div><h3>历史日志推断 · 服务分组</h3><p>仅按中转站历史日志结果推断；展开查看实际承载渠道，不能替代客户端确认或精确协议事实</p></div><span class="muted">${nfmt(d.groups?.length||0)} 个有流量分组</span></div><div class="stability-group-head"><span>服务分组 / 渠道</span><span>日志推断成功率 / 环比</span><span>时间窄条 · ${bucketLabel(d.meta.timeline_bucket_sec||3600)}</span><span>纳入口径请求 / 占比</span><span>问题 / 问题率</span><span>渠道 / 操作</span></div><div id="stGroupList"></div></section>
...
function renderKpis(d){const s=d.summary,p=d.previous,pc=d.meta?.comparison_coverage||{};const k=$('stKpis');if(!k)return;k.innerHTML=[
  ['历史日志推断稳定性',pct(s.stability),delta(d.delta_pp),health(s)],
...
function renderDrawer(){if(!st.drawer)return;if(st.drawer.kind==='delivery'){renderDeliveryDrawer();return}const x=st.drawer.entity;const title=st.drawer.kind==='channel'?`#${x.id} ${x.name}`:x.name;$('stDrawerTitle').textContent=title;$('stDrawerSubtitle').textContent=`${st.drawer.kind==='channel'?'上游渠道 · '+st.drawer.group.name:'服务分组'} · ${st.report.meta.from} 至 ${st.report.meta.to}`;document.querySelectorAll('[data-st-drawer-tab]').forEach(b=>{b.classList.toggle('active',b.dataset.stDrawerTab===st.drawerTab);b.hidden=b.dataset.stDrawerTab==='timeline'});const body=$('stDrawerBody');if(st.drawerTab==='problems'){renderDrawerProblems();return}const nav={days:st.days,group:st.drawer.group.name};if(st.custom){nav.from=st.custom.from;nav.to=st.custom.to;delete nav.days}if(st.drawer.kind==='channel')nav.channel=x.id;const finance=st.drawer.kind==='channel'?`<div class="monitor-cross-actions"><button type="button" onclick="monitorOpenEncoded('channels','${encodeNav(nav)}')">查看使用与倍率配置 →</button></div>`:'';body.innerHTML=`${finance}<section class="stability-drawer-kpis"><div><small>历史日志推断稳定性</small><b>${pct(x.stability)}</b></div><div><small>历史日志请求</small><b>${nfmt(x.requests)}</b></div><div><small>问题请求</small><b>${nfmt(x.problems)}</b></div><div><small>环比变化</small><b class="${deltaClass(x.delta_pp)}">${x.delta_pp==null?'—':(x.delta_pp>=0?'+':'')+x.delta_pp.toFixed(2)+' pp'}</b></div></section><section class="stability-panel-head" style="border:1px solid #30394b;border-bottom:0;border-radius:10px 10px 0 0"><div><h3>每日稳定性曲线</h3><p>仅还原当前所选对象的历史日志推断</p></div></section><div id="stDrawerChart" class="stability-chart"></div><section class="stability-model-list"><div class="stability-panel-head"><div><h3>${st.drawer.kind==='group'?'模型表现':'承载模型'}</h3><p>按历史日志请求量排序</p></div></div>${(x.models||[]).map(m=>`<div class="stability-model-row"><b>${esc(m.name)}</b><span>${pct(m.stability)}</span><small>${nfmt(m.requests)} 请求</small><small>${nfmt(m.problems)} 问题</small></div>`).join('')||'<div class="stability-empty"><p>当前范围无模型数据</p></div>'}</section>`;renderDrawerChart(x.daily||[])}

async function openDeliveryTimeline(request){st.lastFocus=document.activeElement;st.drawer={kind:'delivery',request,data:null};st.drawerTab='run';$('stDrawerMask')?.classList.add('open');$('stDrawer')?.classList.add('open');$('stDrawer')?.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';loading($('stDrawerBody'),'正在读取脱敏证据时间线…');try{const res=await fetch('/stability/delivery-timeline?request='+encodeURIComponent(request),{headers:{Accept:'application/json'}});const d=await res.json();if(!res.ok)throw new Error(d.error||`HTTP ${res.status}`);if(st.drawer?.request===request){st.drawer.data=d;renderDrawer()}}catch(e){if(st.drawer?.request===request)errorBox($('stDrawerBody'),e.message)}}
async function openDeliveryTimelineByRequestID(requestID){st.lastFocus=document.activeElement;st.drawer={kind:'delivery',request:'lookup',data:null};st.drawerTab='run';$('stDrawerMask')?.classList.add('open');$('stDrawer')?.classList.add('open');$('stDrawer')?.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';loading($('stDrawerBody'),'正在本地计算关联值并查询证据…');try{const res=await fetch('/stability/delivery-timeline/lookup',{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json'},body:JSON.stringify({request_id:requestID})});const d=await res.json();if(!res.ok)throw new Error(d.error||`HTTP ${res.status}`);if(st.drawer?.request==='lookup'){st.drawer.request=d.request?.RequestTraceHMAC||d.request?.request_trace_hmac||'lookup';st.drawer.data=d;renderDrawer()}}catch(e){if(st.drawer?.kind==='delivery')errorBox($('stDrawerBody'),e.message)}}
function deliveryTimelineRows(d){return (d.client_evidence||[]).map(x=>({at:+x.occurred_at_ms,type:x.event_type,source:'受控客户端',detail:(x.outcome||'请求开始')+(x.error_signature?` · ${x.error_signature}`:'')})).sort((a,b)=>a.at-b.at)}
function renderDeliveryDrawer(){const d=st.drawer.data;if(!d)return;const r=d.request||{},events=deliveryTimelineRows(d),outcomes=(d.client_evidence||[]).filter(x=>x.event_type==='request_outcome');$('stDrawerTitle').textContent='客户端证据 '+String(r.request_trace_hmac||'').slice(0,12)+'…';$('stDrawerSubtitle').textContent='仅展示 Monitor 已接收的受控客户端事件';document.querySelectorAll('[data-st-drawer-tab]').forEach(b=>{b.hidden=false;b.classList.toggle('active',b.dataset.stDrawerTab===st.drawerTab)});const body=$('stDrawerBody');if(st.drawerTab==='timeline'){body.innerHTML=`<div class="stability-problem-list">${events.map(x=>`<div class="stability-problem-row"><span><b>${esc(x.source)}</b><small>${new Date(x.at).toLocaleString('zh-CN',{hour12:false})}</small></span><code>${esc(x.type)}</code><span><small>${esc(x.detail)}</small></span></div>`).join('')||'<div class="stability-empty">无事件</div>'}</div>`;return}if(st.drawerTab==='problems'){const signatures=outcomes.map(x=>x.error_signature).filter(Boolean);body.innerHTML=signatures.length?`<div class="stability-model-list">${[...new Set(signatures)].map(x=>`<div class="stability-model-row"><b>${esc(x)}</b><small>脱敏后的规范化问题签名</small></div>`).join('')}</div>`:'<div class="stability-empty"><b>没有问题签名</b></div>';return}const latest=outcomes.at(-1);body.innerHTML=`<section class="stability-drawer-kpis"><div><small>客户端终态</small><b>${esc(latest?.outcome||'尚无终态')}</b></div><div><small>事件数</small><b>${nfmt(events.length)}</b></div><div><small>重试序号</small><b>${nfmt(latest?.retry_index||0)}</b></div><div><small>客户端</small><b>${esc(latest?`${latest.client_family}@${latest.client_version}`:'—')}</b></div></section><div class="stability-evidence-note">Monitor 没有修改 NewAPI，这里只陈述客户端主动回传的技术结果。</div>`}
...
async function loadProblems(){if(st.view!=='problems')return;if(st.problemAbort)st.problemAbort.abort();st.problemAbort=new AbortController();loading($('stProblemBody'),'正在汇总问题与证据…');try{const [legacyRes,deliveryRes]=await Promise.all([fetch('/stability/problems?'+queryParams(),{headers:{Accept:'application/json'},signal:st.problemAbort.signal}),fetch('/stability/delivery-issues?'+queryParams({protocol:'openai_responses'}),{headers:{Accept:'application/json'},signal:st.problemAbort.signal})]);if(legacyRes.status===401||deliveryRes.status===401){location.href='/login';return}const legacy=await legacyRes.json(),delivery=await deliveryRes.json();if(!legacyRes.ok)throw new Error(legacy.error||`HTTP ${legacyRes.status}`);if(!deliveryRes.ok)throw new Error(delivery.error||`HTTP ${deliveryRes.status}`);$('stProblemBody').innerHTML=deliveryIssueTable(delivery)+problemTable(legacy,false)}catch(e){if(e.name!=='AbortError')errorBox($('stProblemBody'),e.message)}}
function deliveryIssueTable(d){const rows=d.issues||[];return `<div class="stability-panel-head"><div><h3>受控客户端问题</h3><p>只展示客户端明确回传的技术失败；未接入时不做推断</p></div><span>${nfmt(rows.length)} 条</span></div>${rows.length?`<div class="stability-problem-list">${rows.map(x=>`<button type="button" class="stability-problem-row" data-delivery-request="${esc(x.request_trace_hmac)}"><span><b>${esc(x.outcome||'技术失败')}</b><small>${esc(`${x.client_family}@${x.client_version}`)}</small></span><code>${esc(x.error_signature||'—')}</code><span><small>${esc(x.protocol||'—')} · ${esc(x.model||'—')}</small></span><span class="count">重试 #${nfmt(x.retry_index)}</span><small>${new Date(x.occurred_at_ms).toLocaleString('zh-CN',{hour12:false})}</small></button>`).join('')}</div>`:'<div class="stability-empty"><p>当前范围没有受控客户端失败证据；这不表示全站客户端一定无问题。</p></div>'}`}
function problemTable(d,compactMode){const rows=d.problems||[];let coverage='';if(+d.pending_minutes>0)coverage+=`<div class="alert">历史日志错误采集仍有 ${nfmt(d.pending_minutes)} 个分钟待处理；下列排行只包含已完整采集的分钟，不会把部分数据当成完整结果。</div>`;if(+d.uncovered_minutes>0)coverage+=`<div class="alert">当前日期范围有 ${nfmt(d.uncovered_minutes)} 个分钟尚无历史日志错误覆盖；不能据此判断未覆盖时段没有错误。</div>`;if(!rows.length)return `${coverage}<div class="stability-empty"><b>当前范围没有已采集的历史日志问题</b><p>这不表示客户端或未覆盖时段一定没有问题。</p></div>`;return `${coverage}${!compactMode?`<div class="stability-advice-pending"><b>口径边界：</b>以下是脱敏后的规范化错误签名与影响范围，属于历史日志推断；不自动判断责任方，也不替代客户端确认结果。</div>`:''}<div class="stability-problem-list"><div class="stability-problem-head"><span>来源 / error code</span><span>脱敏问题签名</span><span>影响范围</span><span style="text-align:right">数量</span><span>最后出现</span></div>${rows.map(p=>`<div class="stability-problem-row"><span><b class="code">${esc(p.code||'无明确 code')}</b><small>${esc(p.source)}</small></span><code>${esc(p.message||'(空)')}${p.truncated?' …（已截断）':''}</code><span><small>${esc((p.groups||[]).slice(0,3).join('、')||'—')}<br>${(p.channel_ids||[]).slice(0,4).map(id=>'#'+id).join('、')||'未归属渠道'}</small></span><span class="count">${nfmt(p.count)}</span><small>${dateTime(p.last_ts)}</small></div>`).join('')}</div>${d.truncated?'<div class="alert">问题类型超过接口安全上限，仅显示数量最高的部分。</div>':''}`}
