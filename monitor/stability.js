...
const ST_HEADERS={
  ...
  'group-governance':{title:'分组治理',subtitle:'倍率 · 渠道 · 用户 · 令牌 · 订阅引用的只读审计',icon:'groups'},
  ...
  finance:{title:'经营核算',subtitle:'用户侧消耗 · 修正上游成本 · 贡献毛利 · 数据覆盖',icon:'finance'},
...
const ST_ICONS={
  ...
  groups:'<svg viewBox="0 0 24 24"><circle cx="6" cy="6" r="2"/><circle cx="18" cy="6" r="2"/><circle cx="12" cy="18" r="2"/><path d="M8 7.2l3 8.2M16 7.2l-3 8.2M8 6h8"/></svg>',
  ...
  finance:'<svg viewBox="0 0 24 24"><path d="M4 3h16v18H4z"/><path d="M8 7h8M8 11h2M14 11h2M8 15h2M14 15h2M8 19h2M14 19h2"/></svg>',
...
(function initMonitorSidebar(){
  ...
  // 展开时已有可见文字，不重复显示 tooltip；收起后由 CSS 在持续悬停
  // 1 秒时读取 data-nav-tooltip。移除原生 title，避免浏览器立即弹出。
  shell.querySelectorAll('.monitor-sidebar .monitor-nav .tab').forEach(item=>{
    const text=(item.querySelector('.monitor-nav-label')?.textContent||item.getAttribute('title')||'').trim();
    if(!text)return;
    item.dataset.navTooltip=text;
...
function queryParams(extra){
  ...
  if(st.custom){q.set('from',st.custom.from);q.set('to',st.custom.to)}
  else if(st.hours){q.set('hours',String(st.hours))}
  else q.set('days',String(st.days));
...
function applyNavigationContext(){
  ...
  if(c.from&&c.to){const custom={from:c.from,to:c.to};if(!st.custom||st.custom.from!==custom.from||st.custom.to!==custom.to||st.preset!=='custom'||st.hours){st.custom=custom;st.hours=0;st.preset='custom';changed=true}}
  else if(+c.hours>0&&(st.hours!==+c.hours||st.custom||st.preset)){st.hours=+c.hours;st.custom=null;st.preset='';changed=true}
  else if(+c.days>0&&(st.days!==+c.days||st.hours||st.custom||st.preset)){st.days=+c.days;st.hours=0;st.custom=null;st.preset='';changed=true}
...
function init(){
  ...
  document.querySelectorAll('[data-stability-hours]').forEach(b=>b.addEventListener('click',()=>{st.hours=+b.dataset.stabilityHours;st.custom=null;st.preset='';$('stCustomRange')?.classList.remove('show');syncRange();reloadActiveLayer()}));
  document.querySelectorAll('[data-stability-days]').forEach(b=>b.addEventListener('click',()=>{st.days=+b.dataset.stabilityDays;st.hours=0;st.custom=null;st.preset='';$('stCustomRange')?.classList.remove('show');syncRange();reloadActiveLayer()}));
  document.querySelectorAll('[data-stability-preset]').forEach(b=>b.addEventListener('click',()=>{st.hours=0;st.preset=b.dataset.stabilityPreset;st.custom=stPresetRange(st.preset);$('stCustomRange')?.classList.remove('show');syncRange();reloadActiveLayer()}));
...
const EDGE_ERROR_LABELS={upstream_timeout:'上游响应超时',upstream_connect_failed:'上游连接失败',upstream_closed:'上游提前断开',upstream_tls:'上游 TLS',client_closed:'客户端提前断开',worker_capacity:'Nginx 容量',resolver:'域名解析',rate_limited:'入口限流',request_body:'请求体',other_error:'其他 Nginx 错误'};
function edgeErrorRows(rows){return (rows||[]).map(r=>`<div class="stability-model-row"><b>${esc(EDGE_ERROR_LABELS[r.category]||r.category)}</b><span>${nfmt(r.count)} 次</span><small>级别 ${esc(r.severity||'—')}</small><small>节点侧分类聚合</small></div>`).join('')||'<div class="stability-empty"><p>当前范围没有纳入分类的 Nginx error 事件</p></div>'}
function edgeSourceRows(rows,label){return (rows||[]).map(x=>`<div class="stability-model-row"><b>${esc(x.node)} · ${esc(label)}</b><span>${x.status==='ok'?'正常':x.status==='warn'?'需关注':'中断'}</span><small>最新事件 ${dateTime(x.last_event_ts)} · 送达 ${age(x.age_sec)} 前</small><small>${x.backlog_known?'待读 '+bytes(x.backlog_bytes):'日志/待读状态不可读'} · ${x.cursor_discontinuities?`游标不连续 ${nfmt(x.cursor_discontinuities)} 次，最近 ${dateTime(x.last_cursor_discontinuity_at)}`:'游标连续'} · ${x.discarded_lines?`跳过不合法或超窗日志 ${nfmt(x.discarded_lines)} 行，最近 ${dateTime(x.last_discarded_at)}`:'未跳过日志'}</small></div>`).join('')}
function renderEdge(d){
  ...
  el.innerHTML=`<div class="stability-advice-pending"><b>口径边界：</b>access log 只上传脱敏分钟事实；error log 只上传有限类别、严重级别和分钟计数，原文/IP/URL/upstream 地址均不离开节点。请求级关联只使用经 HMAC 的 Request ID，error 分类不能冒充单请求证据。当前保留 ${nfmt(d.retention_days||7)} 天。</div>`
    +`<section class="stability-kpis"><article class="stability-kpi"><small>入口请求</small><b>${nfmt(s.requests)}</b><em>${esc(d.from||'—')} 至 ${esc(d.to||'—')}</em></article><article class="stability-kpi"><small>HTTP 4xx / 5xx</small><b class="${s.status_5xx?'bad':''}">${nfmt(s.status_4xx)} / ${nfmt(s.status_5xx)}</b><em>客观状态码，不自动归因</em></article><article class="stability-kpi"><small>入口 P95 / P99</small><b>${s.latency_coverage?`${nfmt(s.p95_request_ms)} / ${nfmt(s.p99_request_ms)} ms`:'—'}</b><em>直方图覆盖 ${pct(s.latency_coverage)} · 桶上界估算</em></article><article class="stability-kpi"><small>Request ID 携带率</small><b>${pct(s.request_id_coverage)}</b><em>仅“存在”，不是关联成功率 · ${bytes(s.bytes_sent)}</em></article></section>`
    ...
    +`<section class="stability-panel"><div class="stability-panel-head"><div><h3>Nginx error 分类</h3><p>节点级旁证，不做时间邻近的伪精确请求关联</p></div></div>${edgeErrorRows(d.errors)}</section>`
    +`<section class="stability-panel"><div class="stability-panel-head"><div><h3>采集器状态</h3><p>access 与 error 使用独立游标/重试；任一链故障不会阻塞另一链。</p></div></div>${edgeSourceRows(sources,'access')+edgeSourceRows(d.error_sources||[],'error')||'<div class="stability-empty"><p>已启用但尚未收到采集器数据</p></div>'}</section>`;
...
function dayStrip(points){const rows=points||[];const dense=rows.length>60?' dense':'';const unit=bucketLabel(st.report?.meta?.timeline_bucket_sec||3600);return `<div class="day-strip${dense}">${rows.map(d=>{const cov=d.coverage||'missing';let c=cov==='missing'?'missing':cov==='provisional'?'provisional':cov==='legacy'?'legacy':'empty';let detail=cov==='missing'?'数据缺失':cov==='provisional'?'等待定稿':cov==='legacy'?'旧口径兼容数据':d.requests?nfmt(d.requests)+' 次 · '+pct(d.stability):'已确认无请求';if((cov==='complete_data'||cov==='complete_zero')&&d.requests)c=d.stability>=99?'good':d.stability>=95?'warn':'bad';return `<i class="${c}" title="${esc(dateTime(d.ts))} · ${esc(unit)} · ${esc(detail)}"></i>`}).join('')}</div>`}
function groupRow(g){const deltaValue=st.report?.meta?.comparison_available===true?g.delta_pp:null;return `<div class="stability-group-row" role="button" tabindex="0" data-st-group="${esc(g.name)}"><span class="stability-title"><i class="${health(g)}"></i><span><b>${st.expanded.has(g.name)?'▾':'▸'} ${esc(g.name)}</b><small>${esc(g.vendor||'未标记')} · ${nfmt(g.channels?.length||0)} 个实际渠道 · ${nfmt(g.model_count||g.models?.length||0)} 个模型</small></span></span><span class="stability-rate"><b>${pct(g.stability)}</b><small class="${deltaClass(deltaValue)}">${delta(deltaValue)}</small></span>${dayStrip(g.timeline)}<span class="stability-cell-num"><b>${nfmt(g.requests)}</b><small>${(+g.share_pct||0).toFixed(1)}% 全站</small></span><span class="stability-cell-num"><b>${nfmt(g.problems)}</b><small>${pct(g.problem_rate)}</small></span><span><button type="button" class="stability-open-detail" data-st-detail="group" data-group="${esc(g.name)}">完整详情 →</button></span></div>`}
function channelRow(g,ch){const state=ch.current===false?'历史渠道':ch.status===1?'当前启用':ch.status?'当前停用':'状态未知',deltaValue=st.report?.meta?.comparison_available===true?ch.delta_pp:null;return `<div class="stability-channel-row"><span class="stability-title"><i class="${health(ch)}"></i><span><b>#${ch.id} ${esc(ch.name)}</b><small>${esc(ch.vendor||'未标记')} · ${state} · ${nfmt(ch.model_count||ch.models?.length||0)} 个模型</small></span></span><span class="stability-rate"><b>${pct(ch.stability)}</b><small class="${deltaClass(deltaValue)}">${delta(deltaValue)}</small></span>${dayStrip(ch.timeline)}<span class="stability-cell-num"><b>${nfmt(ch.requests)}</b><small>${(+ch.share_pct||0).toFixed(1)}% 分组</small></span><span class="stability-cell-num"><b>${nfmt(ch.problems)}</b><small>${pct(ch.problem_rate)} · 贡献 ${(+ch.problem_share_pct||0).toFixed(1)}%</small></span><span><button type="button" class="stability-open-detail" data-st-detail="channel" data-group="${esc(g.name)}" data-channel="${ch.id}">完整详情 →</button></span></div>`}
function renderGroups(){const el=$('stGroupList');if(!el)return;const groups=st.report?.groups||[];el.innerHTML=groups.map(g=>{let detail='';if(st.expanded.has(g.name)){detail=st.detailLoading.has(g.name)?'<div class="stability-loading"><i class="stability-spinner"></i><span>正在加载该分组详情…</span></div>':g._detail_loaded?(g.channels||[]).map(ch=>channelRow(g,ch)).join('')+(g.rejected?`<div class="stability-channel-row"><span class="stability-title"><i class="wait"></i><span><b>未归属请求（明确记录）</b><small>选择渠道前结束，不属于任何真实渠道；当前仅保留计数</small></span></span><span class="stability-rate"><b>—</b><small>不计算稳定性</small></span><span></span><span class="stability-cell-num"><b>${nfmt(g.rejected)}</b><small>明确记录</small></span><span class="stability-cell-num"><b>—</b><small>不参与问题数</small></span><span></span></div>`:''):'<div class="stability-empty"><p>详情尚未加载</p></div>'}return groupRow(g)+detail}).join('')||'<div class="stability-empty"><b>当前筛选没有有流量分组</b></div>'}
