...
- NewAPI 使用现有 OFFSET 日志接口：当天只重读最近 3 小时重叠窗口以吸收晚到日志，历史按已闭合小时推进。Monitor 校验每页 `total`、预期行数，并在多页扫描后重读首页指纹；并发变化会使整窗口失败重试。单窗超过 5,000 行时按时间二分，单秒仍超过 5,000 行则 fail-closed。Tail 单轮最多 60 次请求；历史单轮最多 20 次请求且最长 45 秒，随后保存断点并让出调度器。
...
- AICodeWith 按 Key 读取日账单。多把 Key 分批执行、按远端 `api_key_id` 去重，全部 Key 覆盖同一冻结窗口后才原子求和发布；中途失败不会发布部分 Key 的金额。单轮最多处理 4 把 Key，账户内所有 Key 共用 8 秒请求间隔；429、主机熔断、超时或 5xx 会立即停止本轮，未处理 Key 留到下轮。管理端每次最多新增或替换 4 把 Key，只校验变更 Key 和一把已验证基准 Key，不重扫其余旧 Key。历史接口单次最多补 31 个中国自然日。
- NewAPI/Sub2API 默认 Tail 间隔 20 分钟，AICodeWith 至少 30 分钟，均带确定性单向抖动。账户凭据操作按域名隔离，同一账户/同一主机严格串行，后台遇到管理员配置等待时让路。余额、使用日志和计价账本使用独立调度循环；使用日志每 10 秒优先取到期 Tail，为避免持续流量下历史永久饥饿，连续 4 批 Tail 后最多让出 1 批历史；但最旧 Tail 逾期超过 5 分钟时严禁让路。`MONITOR_UPSTREAM_MAX_CONCURRENCY` 默认 1、硬上限 2，只允许不同主机并行；所有请求仍受全局启动间隔和 host 熔断保护。账户 gate 是进程内互斥，启用上游日志/计价 worker 时必须保持单活 Monitor，不得在第二实例同时开启同步。429 优先遵守 `Retry-After`；超时/5xx 按 2、5、15、30、60 分钟渐退；SQLite busy 按 10、30、60、120 秒快速恢复；认证失败只隔离对应账户/Key。管理页“上游账户同步”按账户分开显示余额、当天 Tail、历史补数和计价账本；新开账户明确显示“等待首次同步”，不得用“历史未完成”推断当天不新鲜。AICodeWith 还会只读展示每把 Key 的名称、当天状态与历史退避水位，任何接口都不得返回 Key 内容。
- NewAPI 历史整小时请求若发生请求级超时或 HTTP 408/504/524，会先持久建立子窗口计划，再按 `5 分钟 → 60 秒 → 10 秒` 逐级降档。每个子窗口保留分页断点并复核首页；只有全部子窗口形成连续、无重叠的整小时覆盖后才原子发布。单轮仍受 20 次请求和 45 秒预算限制；父级 45 秒预算耗尽只保留断点并让出调度器，不得误判为来源慢查询继续拆分。10 秒最小窗口仍超时会按失败策略退避，绝不跳过或发布部分小时。
...
- 计价证据账本是既有 `channel_upstream_usage_hours` 的独立影子链路，支持 NewAPI、Sub2API 和 AICodeWith，不覆盖、不修正现有上游消费数据。它保存已脱敏的小时聚合证据，不保存日志 ID、请求内容、IP 或凭据。
- NewAPI 保存标准/用户/生效倍率；Sub2API 保存逐请求已固化的生效倍率与实际扣费，日志里嵌套的 `group` 是当前关联对象，不把它的当前倍率冒充成历史标准倍率；AICodeWith 当前只发布日志明确返回的折扣和实际扣费，能力标记为 `discount_only`，不推断、不冒充完整基础倍率。
- Sub2API 按中国自然日只扫描一次逐请求日志，在 Monitor 本地拆成 24 个小时；分页逐页断点保存。AICodeWith 每轮最多处理 4 把 Key，任一 Key 失败都不发布部分账本。明细达到单次上限而上游未提供已验证分页契约时 fail-closed，不写成“已补齐”。
- 当前阶段只做“观测、对账、记录变化证据”，不会自动改写渠道管理中人工维护的上游倍率/折扣。真实 4sapi 数据完成口径验收、渠道映射无歧义且通知/回滚闭环完成前，禁止开启自动维护。
- 倍率变化事件只表示灰度启用后，按时间向前连续确认到的变化；历史补数保留每小时证据，但不会倒推并宣称已重建完整历史变更链。需要历史变更审计时应直接按小时证据查询。
- NewAPI 的最终 `quota` 按上游精确整数保存；Sub2API/AICodeWith 实际扣费统一换算为百万分之一货币单位的整数证据。倍率/折扣只作为计价证据，不再乘第二次。一个小时必须连续两次完整读取的内容摘要一致；NewAPI/Sub2API 还必须与现有小时账单的请求数、Token 和金额对平，才会更新已确认倍率状态。AICodeWith 既有账单是自然日聚合，小时折扣证据只做两次源端一致性确认，不伪造小时分摊对账。
- 首次部署保持 `MONITOR_UPSTREAM_PRICING_LEDGER_ENABLED=false`。验证 Monitor/Usage 原有功能后，再设为 `true`，同时将 `MONITOR_UPSTREAM_PRICING_LEDGER_DOMAINS=4sapi.com`、`MONITOR_UPSTREAM_PRICING_BACKFILL_HOURS_PER_RUN=1`。它使用独立 worker 锁，不占用既有余额/消费同步的粗粒度互斥；与旧链路仍共享按 host 的请求级节流和熔断，避免重复读取把上游打满。因此慢响应或 429 仍可能让该主机的后台余额/账单同步延后，这不影响 NewAPI 用户请求，但灰度时必须观察旧 `usage_status`、余额水位、Host Circuit 与 429。影子任务每轮最多 20 次请求并按页持久化断点，首期每分钟最多选一个账户；其账本错误不改写旧账单健康。
- “数据同步状态”显示近期核对水位、历史已确认/待确认小时、差异小时和最近错误。只有两次读取一致且对平的小时才计入已确认；不得以“进度 100%”替代金额核对。
...
- 资金流水是人工充值记账的证据补充，不会自动改写“上游充值支付/到账”、渠道倍率或正式利润报表。账户级充值不按本地实际渠道复制，只关联当前归并主域名，避免一笔钱被重复统计。
- 首版只启用已核对的 NewAPI `/api/log/self` 契约，分别读取 `type=1/3/6`，将主动充值、兑换码、管理员增加/扣减/覆盖额度、消费退款单独入账。Sub2API、AICodeWith 和 TokenForce 在真实响应 fixture 通过前显示“待适配”，不把未采集显示成 0。
- 到账美元、实付原币、管理净调整和消费退款分列；原文未声明实付币种时标为“原币未知”，禁止与 USD 到账额度求和。无法解析金额的事件仍保留原文与未知计数，不得丢弃或猜值。
- 未修改 NewAPI 的日志 `id` 是页内序号，不能用作幂等键。Monitor 使用排除伪 ID 的规范化事件摘要，同秒内完全相同的事件保留 `observed_count`；重叠重扫只取已观测最大重数，不会重复入账。
- 分页必须校验 `total`、页行数和扫描后首页指纹。窗口过密时二分续传，失败不落半页、不推进水位。明细页最多显示 500 条，但汇总 SQL 始终覆盖整个查询区间。
- 必须同时满足 `MONITOR_UPSTREAM_FUNDS_SYNC_ENABLED=true`、非空 `MONITOR_UPSTREAM_FUNDS_DOMAINS`、账户已开启日志读取授权。第一次发布保持总开关关闭；灰度时一次只加一个 NewAPI 主域名，`MONITOR_UPSTREAM_FUNDS_BACKFILL_DAYS` 默认 90，历史每轮只推进一天。
- NewAPI 普通用户的 self 日志可能看不到“管理员记在操作人名下”的代调额度。所以当前版本不得宣称可以替代人工记账；后续应用余额快照 + 已知消费 + 已知资金事件做差额对账，只将无法解释的差额标为“未归因调整”，不猜测为充值。
